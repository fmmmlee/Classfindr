...
public class UploadToAWS implements Runnable{
	...
	//TODO: Add metrics to upload functions
	//Also add a counter that checks how many uploads per second are going; need to see if distributing this task or batching will help at all
	//check at beginning of while loop is 1 behind I think; it doesn't catch after the last upload
    ...
    private static void item_put(String tableName, HashMap<String, AttributeValue> toPush)
    {
    	final AmazonDynamoDB ddb = AmazonDynamoDBClientBuilder.defaultClient();
    		...
    		try {
                ...
                job_progress++;
                if(job_size.get() != 0){
                	System.out.println(job_progress + "/" + job_size + "[" + StringUtils.repeat('|', (int) (50.0*((double) job_progress/((double) job_size.get())))) + StringUtils.repeat(' ', (int) (50.0*(1.0-((double) job_progress/((double) job_size.get()))))) + "]");
                }
